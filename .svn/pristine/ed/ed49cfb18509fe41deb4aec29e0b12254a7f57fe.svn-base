...
do
  databases=$(mysql -h$server -u$dbuser -p$dbpass -Bse 'show databases;')
  ...
  do
    ...
    then
      ...
      mysqldump -h$server -u$dbuser -p$dbpass $database > $mysqlbackuppath
      finishtime=`date`
...
do
  folders=$(mysql -u$dbuser -p$dbpass -hispcfg01.skdk.internal scripts_data -Bse "select folder from backup_dirs where server = '$server'")
